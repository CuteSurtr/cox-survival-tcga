...
def run(project_id: str, out_root: Path) -> dict:
    ...
    X_df['stage_clean'] = clin_raw.set_index('submitter_id').loc[X_df['submitter_id'], 'stage'].map(harmonise_stage).values
    ...
    model = CoxPH(max_iter=200, tol=1e-09).fit(X, t, e)
    print('\nCox PH coefficients:')
    ...
    plot_km_by_stage(X_df.assign(os_time_days=t, os_event=e), fig_dir / f'{project_id}_km_by_stage.png', title=f'{project_id}: Kaplan-Meier by AJCC stage')
    ...
    plot_hazard_ratios(summary_df, fig_dir / f'{project_id}_forest.png', title=f'{project_id}: Cox PH hazard ratios')
    ...
    return {'n_cases': int(len(X_df)), 'cox_summary': summary_df, 'c_index': float(c), 'logrank_chi2': float(lr['chi2']), 'logrank_p': float(lr['p_value'])}
