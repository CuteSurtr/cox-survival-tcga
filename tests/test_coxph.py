...
def test_coxph_agrees_with_lifelines_on_simulated_data():
    ...
    ours = CoxPH(max_iter=100, tol=1e-08).fit(X, t, e)
    df = pd.DataFrame(X, columns=[f'x{i}' for i in range(X.shape[1])])
    df['T'] = t
    df['E'] = e
    lf = CoxPHFitter(penalizer=0.0).fit(df, duration_col='T', event_col='E')
    lf_beta = lf.params_.values
    (np.testing.assert_allclose(ours.beta_, lf_beta, atol=0.005), f'beta mismatch: ours={ours.beta_}, lifelines={lf_beta}')
    ours_se = ours.standard_errors()
    lf_se = lf.standard_errors_.values
    np.testing.assert_allclose(ours_se, lf_se, rtol=0.01)

def test_coxph_recovers_true_parameters():
    X, t, e, beta_true = _simulate(n=3000, p=3, seed=2)
    ours = CoxPH(max_iter=200, tol=1e-09).fit(X, t, e)
...
def test_coxph_handles_light_ties():
    ...
    ours = CoxPH(max_iter=100, tol=1e-08).fit(X, t_rounded, e)
    df = pd.DataFrame(X, columns=['x0', 'x1'])
    df['T'] = t_rounded
    df['E'] = e
    lf = CoxPHFitter().fit(df, duration_col='T', event_col='E')
    np.testing.assert_allclose(ours.beta_, lf.params_.values, atol=0.01)

def test_coxph_heavy_ties_stable():
    X, t, e, _ = _simulate(n=400, p=2, seed=3)
    t_heavy = np.round(t, 1)
    ours = CoxPH(max_iter=100, tol=1e-08).fit(X, t_heavy, e)
    df = pd.DataFrame(X, columns=['x0', 'x1'])
    df['T'] = t_heavy
    df['E'] = e
    lf = CoxPHFitter().fit(df, duration_col='T', event_col='E')
    np.testing.assert_allclose(ours.beta_, lf.params_.values, atol=0.05)
    assert np.isfinite(ours.log_lik_history_).all()
    diffs = np.diff(ours.log_lik_history_)
    assert (diffs >= -1e-06).all(), 'log-lik should be non-decreasing under Newton-Raphson'
...
def test_km_matches_lifelines():
    ...
    np.testing.assert_allclose(ours.survival_, lf_s, atol=1e-09)
...
def test_logrank_matches_lifelines_two_groups():
    ...
    assert abs(ours['chi2'] - lf.test_statistic) < 1e-06
    assert abs(ours['p_value'] - lf.p_value) < 1e-06
